...
def admin_lockout(
    ...
    return detection.Rule(
        ...
        + [
            match_filters.deep_equal("action", "admin_lockout")
        ],
        alert_title=_title,
        threshold=1,
        unit_tests=(
            [
                detection.JSONUnitTest(
                    ...
                    expect_match=True,
                    data=sample_logs.admin_lockout__invalid_json
                ),
                detection.JSONUnitTest(
                    name="Admin lockout- valid json",
                    expect_match=True,
                    data=sample_logs.admin_lockout__valid_json
                ),
                detection.JSONUnitTest(
                    name="Bypass Create",
                    expect_match=False,
                    data=sample_logs.bypass_create
                ),
